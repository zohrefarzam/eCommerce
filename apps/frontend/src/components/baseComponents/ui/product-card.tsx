      ...
      <div className="relative aspect-square w-full shrink-0 overflow-hidden bg-surface-tertiary/50">
        {image ? (
          <Image
            ...
            fit="contain"
            className="p-6 transition-transform duration-500 ease-out group-hover:scale-105"
          />
        ) : (
        ...
        <div
          className="pointer-events-none absolute inset-0 bg-gradient-to-t from-black/55 via-black/15 to-transparent opacity-90 transition-opacity duration-300 sm:opacity-75 sm:group-hover:opacity-95"
          aria-hidden
        />
        ...
        >
          <Button
            ...
            onPress={onBuy}
            className={[
              'rounded-none bg-foreground/95 py-3 text-sm font-semibold text-background',
              'ring-1 ring-white/20 shadow-xl shadow-black/60 backdrop-blur-sm',
              'transition-[background-color,box-shadow] duration-300',
              'hover:bg-foreground hover:shadow-2xl hover:shadow-black/70 active:bg-foreground/90',
            ].join(' ')}
          >
            {buyLabel}
          </Button>
